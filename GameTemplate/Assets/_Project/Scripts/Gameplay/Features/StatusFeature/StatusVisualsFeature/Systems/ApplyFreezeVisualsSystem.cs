...
namespace _Project.Scripts.Gameplay.Features.StatusFeature.StatusVisualsFeature.Systems
{
    ...
    public sealed class ApplyFreezeVisualsSystem : ISystem
    {
        ...
        public void OnUpdate(float deltaTime)
        {
            foreach (Entity status in _appliedStatuses)
            {
                ...
                if (target.Value.Has<StatusVisualsReference>())
                {
                    ...
                    ref readonly StatusAppliedEvent appliedEvent = ref status.GetComponent<StatusAppliedEvent>();

                    statusVisuals.Value.ApplyFreeze(appliedEvent.StackCount);
                }
            }
        }
