...
namespace _Project.Scripts.Gameplay.Features.InteractiveObjectsFeature.Systems
{
    ...
    public sealed class EnemyEmplacementDestructionSystem : ISystem
    {
        ...
        public EnemyEmplacementDestructionSystem(
        ...
        public void OnUpdate(float deltaTime)
        {
            foreach (DamagedEvent damagedEvent in _damagedEvent.publishedChanges)
            {
                if (damagedEvent.Damageable.IsNullOrDisposed() ||
                    !damagedEvent.Damageable.Has<EnemyEmplacementTag>() ||
                    !damagedEvent.Damageable.Has<ZeroHealthMarker>() ||
                    damagedEvent.Damageable.Has<EnemyEmplacementDestroyingMarker>())
                    continue;

                Entity emplacement = damagedEvent.Damageable;

                emplacement.AddComponent<EnemyEmplacementDestroyingMarker>();

                EntityViewLink viewLink = emplacement.GetComponent<EntityViewLink>();

                ref ColliderLink colliderLink = ref emplacement.GetComponent<ColliderLink>();
                colliderLink.Value.enabled = false;

                ref readonly RewardsLink rewardsLink = ref emplacement.GetComponent<RewardsLink>();
                ...
                ref readonly EnemyEmplacementState state = ref emplacement.GetComponent<EnemyEmplacementState>();
                
                foreach (Entity enemy in state.Enemies) 
                    DestroyEnemy(enemy);

                EnemyEmplacementView emplacementView = (EnemyEmplacementView)viewLink.View;
        ...
        private static void DestroyEnemy(Entity enemy) => 
            World.Default.RemoveEntity(enemy);
